...
void test_parse_false() {
  auto result = json::json_parse("false");
  assert(result);
  assert(std::holds_alternative<json::Boolean>((*result).value));
  assert(!std::get<json::Boolean>((*result).value));
}
...
void test_parse_float_helper(std::string json_str, json::Boolean expected) {
  ...
  auto result = json::json_parse(json_str);
  assert(result);
...
void test_parse_integer_too_big() {
  auto result = json::json_parse("100000000000000000000000000000000000000000000000");
  assert(!result);
  assert(result.error() == json::JsonError::parse_number_too_big);
}
...
void test_parse_string_invalid_escape() {
  auto result = json::json_parse(R"("abc\k")");
  assert(!result);
  assert(result.error() == json::JsonError::parse_invalid_value);
}

void test_parse_string_invalid_unicode_hex() {
  auto result = json::json_parse(R"("abc\u123k")");
  assert(!result);
  assert(result.error() == json::JsonError::parse_invalid_value);
}

void test_parse_string_invalid_unicode_surrogate() {
  auto result = json::json_parse(R"("abc\u1234\u5678")");
  assert(!result);
  assert(result.error() == json::JsonError::parse_invalid_value);
}

void test_parse_string_missing_quotation_mark() {
  auto result = json::json_parse(R"("abc)");
  assert(!result);
  assert(result.error() == json::JsonError::parse_invalid_value);
}
...
void test_parse_array() {
  auto result =
      json::json_parse(R"([null, true, false, 123, "abc", [1, 2, 3], {"a": 1, "b": 2, "c": 3}])");
  assert(result);
...
void test_parse_object() {
  auto result = json::json_parse(R"({"a": 1, "b": 2, "c": 3})");
  assert(result);
...
void test_parse_object_miss_key() {
  auto result = json::json_parse(R"({"a": 1, "b": 2, : 3})");
  assert(!result);
  assert(result.error() == json::JsonError::parse_invalid_value);
}
...
void test_generate_null() {
  auto result = json::json_generate(json::Node{json::Null{}});
  assert(result == "null");
}

void test_generate_true() {
  auto result = json::json_generate(json::Node{true});
  assert(result == "true");
}

void test_generate_false() {
  auto result = json::json_generate(json::Node{false});
  assert(result == "false");
}

void test_generate_integer() {
  auto result = json::json_generate(json::Node{123});
  assert(result == "123");
}

void test_generate_string() {
  auto result = json::json_generate(json::Node{"abc"});
  assert(result == R"("abc")");
}

void test_generate_array() {
  auto result = json::json_generate(json::Node{json::Array{
      json::Node{json::Null{}},
      json::Node{true},
      json::Node{false},
      json::Node{123},
...
void test_generate_object() {
  auto result = json::json_generate(json::Node{json::Object{
      {"n", json::Node{json::Null{}}},
      {"t", json::Node{true}},
      {"f", json::Node{false}},
      {"i", json::Node{123}},
